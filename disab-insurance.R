...
#Tafeln
#SterbeWS Aktiver M & W & D (x bzw. y bzw. d) (hier DAV 2008 T, müsste aber DAV 2004 R sein!)
...
#p() berechnet die 1-Schritt-Übergangsmatrix p zum alter x. Eine vektorisierte version ohne loop ist bislang nicht wirklich gelungen...
p <- function(x, g = "u"){
  if(g == "w"){
    ...
    return(d)
      ...
      u <- c(q_xya$q_x[x], unlist(q_xyi[x, 14:19], use.names = FALSE),1.0000000) #spalte ganz rechts
...
#Plott einer Populationsentwicklung ohne Markovschen Prozess (vgl. Kahlenberg)
#
#+ geom_line(col= "blue", aes(X1, y= X3)) + geom_line(col= "green", aes(X1, y= X4)) +
#
#  ylab("Anzahl") 
#Plotte die Populationsentwicklung mit i_x * 1.2 und r_x *0.8
#r_xyi <- 0.8 * r_xyi
#i_xy <- 1.2 * i_xy
#y <- rbind(1:(m+1),hm(x,m)) 
#y_collapsed <- rbind(1:(m+1), y[2,], colSums(y[3:8,]), y[9,])
#ydata <- data.frame(t(y_collapsed))

#ydata_melted <- melt(ydata, measure=c("X1","X2","X3"))
#plot(x, ydata[1,], col="blue", ylim=c(0,max(ydata[2:8,])))
#for (i in 2:8) points(x, ydata[i,], col=terrain.colors(i))
#ggplot(data = ydata, mapping = aes(x = V1, y =V3))  + geom_line() + geom_point(aes(x = V1, y = V4)) + geom_line() + geom_point(size = 2) + geom_point(aes(x = V1, y = V5)) + geom_point(aes(x = V1, y = V6)) + geom_point(aes(x = V1, y = V7)) + geom_point(aes(x = V1, y = V8)) + geom_point(aes(x = V1, y = u))

#Plot der Tafeln 
#hier soll man sehen wie die aussagen man global treffen kann, z.B. inval nimmt mit steigendem alter zu; welche rolle das geschlecht spielt usw   
#Inval
ggi <- NULL
ggi <- ggplot() + 
...
#qi_diff <- data.frame(t(rbind(15:(length(q_xyi$q_xxi)+14),q_xyi$q_xxi-q_xyi$q_yyi, 
#      q_xyi$q_xm1xi-q_xyi$q_ym1yi, 
#      q_xyi$q_xm2xi-q_xyi$q_ym2yi, 
#      q_xyi$q_xm3xi-q_xyi$q_ym3yi, 
#      q_xyi$q_xm4xi-q_xyi$q_ym4yi,
#      q_xyi$q_xi-q_xyi$q_yi)))

#ggqi_diff <- NULL
#ggqi_diff <- ggqa_diff +
#  geom_line(data = qi_diff, aes(x=X1, y=X2, col ="SterbeWS(Max)")) + 
  #geom_line(data = qi_diff, aes(x=X1, y=X3,col ="black")) +
  #geom_line(data = qi_diff, aes(x=X1, y=X4,col ="black")) + 
  #geom_line(data = qi_diff, aes(x=X1, y=X5,col ="black")) + 
  #geom_line(data = qi_diff, aes(x=X1, y=X6,col ="black")) + 
#  geom_line(data = qi_diff, aes(x=X1, y=X7,col ="SterbeWS(Min)"))
...
#ggr_diff <- NULL
#ggr_diff <- ggqi_diff + 
#  geom_line(data = r_xyi, aes(x=alter, y=(r_xx-r_yy), col ="ReaktWS(Max)")) + 
  #geom_line(data = r_xyi, aes(x=alter, y=(r_xm1x-r_ym1y),col ="black")) +
  #geom_line(data = r_xyi, aes(x=alter, y=(r_xm2x-r_ym2y),col ="black")) + 
  #geom_line(data = r_xyi, aes(x=alter, y=(r_xm3x-r_ym3y),col ="black")) + 
#  geom_line(data = r_xyi, aes(x=alter, y=(r_xm4x-r_ym4y),col ="ReaktWS(Min)")) + 
  #geom_line(data = r_xyi, aes(x=alter, y=(r_x-r_y),col ="black")) +
#  scale_color_manual(name="Legende:",
#                   breaks= c("InvalWS","SterbeWS(A)","SterbeWS(Max)","SterbeWS(Min)","ReaktWS(Max)","ReaktWS(Min)"),
#                   values =c("red","darkcyan","green","darkgreen","blue","deepskyblue")) +
#  ggtitle("Differenz Männlich/Weiblich") + 
#  scale_y_continuous(breaks = seq(-0.0025,0.0175,by=0.0025))
...
#für l = 1 wird 0 ausgegeben. Müsstze das nicht 1 sein?
ai <- function(z,x,l){
  a <- 1
  e <- rep(0,8)
  if(l == 0){
...
#aivec[2,] <- aivec[2,] * i_xy$i_d
#airpdata <- data.frame(t(aivec))
...
ggnep <- ggplot() + 
  geom_point(data = aaidata, aes(V1,aaivec, colour = "NEP-M")) + 
  geom_point(data = aaidata, aes(V1,aaikvec, colour = "NEP-NM")) + 
...
ggnettop <- ggplot() + 
  geom_point(data = nettopdata, aes(V1,aaivec, colour = "NP-M")) + 
  geom_point(data = nettopdata, aes(V1,aaikvec, colour = "NP-NM")) + 
...
#omega max anzahl 55 (entspricht das 67 lebnensjahr, konstante zahl), t max dauer, x startalter
x <- 6
t <- 10
omega <- 55
l <- omega-t+1
...
#berechne 25 bis 36
#berechnung mit kommutationszahlen im sinne des Markovschen Prozesses
for (j in 1:7){
  c <- NULL
  for (i in x:omega) c <- c(c, (a0 %*% p(i))[j] * v^(i))
  c <- sum(c)/c[j]
  print(c) 
}

#berechnung wie ich das gemacht hätte 
  s <- 0 
  for (j in x:omega){
    s <- s + v^j*(rep(1,8) %*% p(j))[1]
    print(j)
  } 
  print(s) 

bw <- 0
#berechnung wie das Koller gemacht hätte 
for (i in x:omega) bw <- bw + lxi * p(i)[1,1] * v^i
bw <- bw/(lxi * p(x)[1,1] * v^x)
print(bw)

bw <- 1
#berechnung wie das Koller gemacht hätte 
for (i in x:omega) bw <- bw + p(i)[1,1] * v^i
print(bw)